...
# learning rate
tf.app.flags.DEFINE_float('learning_rate', 5e-2, '')
...
def main(_):
  ...
  with tf.train.MonitoredTrainingSession() as sess:
    ...
    for tn_batch in range(tn_num_batch):
      ...
      if tn_batch % 4 == 0:
        tn_image_np = tl.prepro.elastic_transform_multi(
            tn_image_np.reshape([flags.batch_size, flags.image_size, flags.image_size]),
            alpha=1,
            sigma=0.04)
      if tn_batch % 4 == 1:
        tn_image_np = tl.prepro.rotation_multi(
            tn_image_np.reshape([flags.batch_size, flags.image_size, flags.image_size, flags.channels]),
            rg=40,
            is_random=True)
      if tn_batch % 4 == 2:
        tn_image_np = tl.prepro.rotation_multi(
        ...
        tn_image_np = tl.prepro.elastic_transform_multi(
            ...
            alpha=1,
            sigma=0.04)
      tn_image_np = tn_image_np.reshape((-1, flags.image_size * flags.image_size * flags.channels))
