...
class TCH():
  def __init__(self, flags, dataset, is_training=True):
    ...
    with tf.variable_scope(tch_scope) as scope:
      ...
      encoded_labels = slim.one_hot_encoding(self.hard_label_ph, dataset.num_label)
      tf.losses.sigmoid_cross_entropy(encoded_labels, self.logits)
      pre_losses = []
      for loss in tf.get_collection(tf.GraphKeys.LOSSES):
        if not loss.name.startswith(tch_scope):
          continue
        print('%-50s added to TCH loss' % loss.name)
      ...
      for regularization_loss in tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES):
        if not regularization_loss.name.startswith(tch_scope):
          continue
        print('%-50s added to TCH loss' % regularization_loss.name)
