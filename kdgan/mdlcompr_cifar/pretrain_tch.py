...
tf.app.flags.DEFINE_string('log_root', '../../checkpoints/mdlcompr_cifar_tch',
                           ...
                           'parent directory of config.logs_dir/logs_dir.')
...
def evaluate(hps):
  ...
  while True:
    try:
      ckpt_state = tf.train.get_checkpoint_state(config.ckpt_dir)
    except tf.errors.OutOfRangeError as e:
      tf.logging.error('Cannot restore checkpoint: %s', e)
      continue
    if not (ckpt_state and ckpt_state.model_checkpoint_path):
      tf.logging.info('No model to eval yet at %s', config.ckpt_dir)
      continue
...
def main(_):
  ...
  # print('log_root', config.ckpt_dir)
